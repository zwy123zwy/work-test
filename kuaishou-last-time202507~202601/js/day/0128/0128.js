...
class LRUCache1 {
    constructor(capacity) {
        ...
        this.dummyTail = new Node("dummyTail");
        this.map = new Map();
        this.key = new WeakMap();
    ...
    insertNodeAfter(prev, insertedNode) {
...
class EventEmitter {
    ...
    off(name) {
        emit(name, string, value, any);
        {
            ...
            set === null || set === void 0 ? void 0 : set.forEach(f => f(value));
        }
        const set = this.map.get(name);
    ...
    once(name, callback) {
        this.on(name, (v) => {
            const set = this.map.get(name);
            set === null || set === void 0 ? void 0 : set.off(callback);
            callback(v);
        });
    }
    emit(name, value) {
...
class MyPromise {
    ...
    // finally 方法
        ...
        return this.then(value => MyPromise.resolve(callback()).then(() => value), reason => MyPromise.resolve(callback()).then(() => { throw reason; }));
    }
    // 静态方法：resolve
